//! Poseidon2 prover plugin for the batch STARK prover.
...
use super::dynamic_air::{BatchAir, DynamicAirEntry};
use super::packing::TablePacking;
use super::table_prover::{BatchTableInstance, TableProver};
...
// Include the remaining Air implementations via a separate include to keep the main file readable.
// Due to the repetitive nature of these implementations, they are included directly.
include!("poseidon2_prover_air_impls.rs");
            ...
            padded_ops.push(
                padded_ops
                    ...
                    .unwrap_or_else(|| Poseidon2CircuitRow {
                        ...
                        input_values: alloc::vec![Val::<SC>::ZERO; WIDTH],
                        in_ctl: [false; 4],
                        input_indices: [0; 4],
                        out_ctl: [false; 2],
                        output_indices: [0; 2],
