    ...
    MVal<SC>: StarkField,
{
        ...
        EF: Field + BasedVectorSpace<MVal<SC>>,
    {
        ...
        let witness_air = WitnessAir::<MVal<SC>, D>::new(witness_rows, witness_lanes);
        let witness_matrix: RowMajorMatrix<MVal<SC>> =
            WitnessAir::<MVal<SC>, D>::trace_to_matrix(&traces.witness_trace, witness_lanes);
    ...
    fn verify<const D: usize>(
        ...
        let witness_air = CircuitTableAir::Witness(WitnessAir::<MVal<SC>, D>::new(
            proof.rows[Table::Witness],
            witness_lanes,
        ));
        let const_air =
