//! BLAKE3 compression table prover: builds [`Blake3CompressAir`] instances for the batch STARK prover.
...
use p3_circuit::ops::{Blake3CompressTrace, NonPrimitivePreprocessedMap, NpoTypeId};
...
use crate::air::blake3_air::{BLAKE3_PREP_OP_WIDTH, Blake3CompressAir};
...
impl<SC, const D: usize> BatchAir<SC> for Blake3CompressAir<Val<SC>, D>
where
...
/// Table prover for BLAKE3 compressions, one call per row.
#[derive(Clone, Copy, Debug, Default)]
pub struct Blake3CompressProver<const D: usize>;

impl<const D: usize> Blake3CompressProver<D> {
            ...
            Algebra<SymbolicExpression<Val<SC>>> + Algebra<SC::Challenge>,
    {
        ...
        let t = trace.as_any().downcast_ref::<Blake3CompressTrace>()?;
        ...
        let mut preprocessed = Val::<SC>::zero_vec(num_ops * BLAKE3_PREP_OP_WIDTH);
        for (op, row) in t.operations.iter().enumerate() {
            let base = op * BLAKE3_PREP_OP_WIDTH;
        ...
        let height = Blake3CompressAir::<Val<SC>, D>::height_for(num_ops, min_height);
        let air = Blake3CompressAir::<Val<SC>, D>::new_with_preprocessed(preprocessed, min_height);
        let matrix = Blake3CompressAir::<Val<SC>, D>::trace_to_matrix(&t.operations, height);
...
impl<SC, const D: usize> TableProver<SC> for Blake3CompressProver<D>
where
    SC: StarkGenericConfig + 'static + Send + Sync,
    Val<SC>: StarkField,
    SymbolicExpressionExt<Val<SC>, SC::Challenge>:
        Algebra<SymbolicExpression<Val<SC>>> + Algebra<SC::Challenge>,
{
    ...
    fn batch_air_from_table_entry(
        ...
        Ok(DynamicAirEntry::new(Box::new(Blake3CompressAir::<
            Val<SC>,
            D,
    ...
    fn air_with_committed_preprocessed(
        ...
        Some(DynamicAirEntry::new(Box::new(Blake3CompressAir::<
            Val<SC>,
            D,
        >::new_with_preprocessed(
...
/// NpoPreprocessor for the BLAKE3 compression table.
///
...
#[derive(Clone, Copy, Debug, Default)]
pub struct Blake3CompressPreprocessor;

macro_rules! blake3_preprocessor_for {
    ($field:ty, $(($ext:ty, $d:literal)),+ $(,)?) => {
        impl NpoPreprocessor<$field> for Blake3CompressPreprocessor {
            fn preprocess(
                &self,
                ...
                $(
                    if let Some(prep) = preprocessed.downcast_mut::<PreprocessedColumns<$ext, $d>>() {
                        return blake3_preprocess_impl::<$field, _, $d>(prep);
                    }
                )+
                Ok(HashMap::new())
            }
...
fn blake3_preprocess_impl<F, EF, const D: usize>(
    ...
    EF: Field + ExtensionField<F> + 'static,
{
    ...
    let mut prep_base: Vec<F> = ef_data
        ...
        .collect::<Result<_, _>>()?;
    if !prep_base.len().is_multiple_of(BLAKE3_PREP_OP_WIDTH) {
...
/// NpoAirBuilder for the BLAKE3 compression table.
#[derive(Clone, Copy, Debug, Default)]
pub struct Blake3CompressAirBuilder<const D: usize>;

impl<SC, const D: usize> NpoAirBuilder<SC, D> for Blake3CompressAirBuilder<D>
where
    SC: StarkGenericConfig + 'static + Send + Sync,
    Val<SC>: StarkField,
    SymbolicExpressionExt<Val<SC>, SC::Challenge>:
        Algebra<SymbolicExpression<Val<SC>>> + Algebra<SC::Challenge>,
{
    fn try_build(
        ...
        if *op_type != NpoTypeId::blake3_compress() {
            return None;
        }
        let num_ops = prep_base.len() / BLAKE3_PREP_OP_WIDTH;
        let height = Blake3CompressAir::<Val<SC>, D>::height_for(num_ops, min_height);
        let air =
            Blake3CompressAir::<Val<SC>, D>::new_with_preprocessed(prep_base.to_vec(), min_height);
        Some((
    ...
    fn try_build_trusted(
        ...
        Some(BuiltNpoTable::new(
            ...
            NpoRelation::new(
                op_type.clone(),
                prep_base.len() / BLAKE3_PREP_OP_WIDTH,
                1,
                AirVariant::Baseline,
                Vec::new(),
            ),
