...
impl NpoTypeId {
    ...
    /// Convenience: OpenInput operation type ID for a given proof extension degree `D`.
    ///
    /// `D` is the extension degree of the proof being verified (not the verification circuit's
    /// own extension degree). This is encoded in the type ID so that the correct `OpenInputAir<F, D>`
    /// can be selected during proving.
    pub fn open_input_d(d: usize) -> Self {
        Self::new(alloc::format!("open_input/d{d}"))
    }

    /// Convenience: OpenInput operation type ID (legacy, assumes D=1).
    #[deprecated(note = "Use open_input_d(D) to encode the proof extension degree")]
