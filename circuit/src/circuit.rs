                ...
                Op::NonPrimitiveOpWithExecutor {
                    ...
                    match executor.op_type() {
                        ...
                        NonPrimitiveOpType::OpenInput => {
                            // Track OpenInput outputs (ro when is_last) in defined[]
                            // so subsequent operations that read ro_wid correctly see it
                            // as already defined and register as readers.
                            for out_limb in outputs {
                                for wid in out_limb {
                                    let wid_idx = wid.0 as usize;
                                    if wid_idx >= defined.len() {
                                        defined.resize(wid_idx + 1, false);
