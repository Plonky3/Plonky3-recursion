...
def build_svg(
    airs: list[dict], layer: int, n: Optional[int], example: str = "recursive_fibonacci"
) -> str:
    airs = enrich(airs)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("-n", "--n", type=int, default=10000,
                   help="Fibonacci index F(n) — only used for recursive_fibonacci (default: 10000)")
    _fwd = "Forwarded to the example if set; otherwise the example uses its own default."
    p.add_argument(
        "--num-recursive-layers", type=int, default=argparse.SUPPRESS, help=_fwd,
    )
    p.add_argument(
        "--log-blowup", type=int, default=argparse.SUPPRESS, help=_fwd,
    )
    p.add_argument(
        "--max-log-arity", type=int, default=argparse.SUPPRESS, help=_fwd,
    )
    p.add_argument(
        "--log-final-poly-len", type=int, default=argparse.SUPPRESS, help=_fwd,
    )
    p.add_argument(
        "--query-pow-bits", type=int, default=argparse.SUPPRESS, help=_fwd,
    )
...
def main() -> int:
    ...
    if args.skip_run:
        ...
        try:
            ...
            for flag, attr in (
                ("--num-recursive-layers", "num_recursive_layers"),
                ("--log-blowup", "log_blowup"),
                ("--max-log-arity", "max_log_arity"),
                ("--log-final-poly-len", "log_final_poly_len"),
                ("--query-pow-bits", "query_pow_bits"),
            ):
                if hasattr(args, attr):
                    cmd.extend([flag, str(getattr(args, attr))])
            cmd.extend(["--emit-trace-json", json_path])
