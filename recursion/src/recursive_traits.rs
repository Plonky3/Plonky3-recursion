...
pub trait RecursiveExtensionMmcs<F: Field, EF: ExtensionField<F>, const D: usize> {
    type Input: Mmcs<EF>;

    type Commitment: Recursive<F, D, Input = <Self::Input as Mmcs<EF>>::Commitment> + Clone;
    type Proof: Recursive<F, D, Input = <Self::Input as Mmcs<EF>>::Proof> + Clone;
}
...
> Recursive<Val<SC>, D> for ProofWires<SC, Comm, OpeningProof, D>
{
    ...
    fn lens(input: &Self::Input) -> impl Iterator<Item = usize> {
        ...
        all_lens.extend(CommitmentWires::<Val<SC>, Comm, D>::lens(commitments));
        all_lens.extend(OpenedValuesWires::<SC, D>::lens(opened_values));
