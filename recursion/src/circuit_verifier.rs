...
use crate::Target;
use crate::challenge_builder::BaseStarkChallenges;
...
/// Getter for all the challenge targets.
///
/// Returns a structured representation of challenges:
/// - Base STARK challenges (alpha, zeta, zeta_next) via `BaseStarkChallenges::generate()`
/// - PCS-specific challenges via `RecursivePcs::get_challenges_circuit()`
///
/// Returns flat vector for backward compatibility with existing code.
...
where
    SC::Pcs: RecursivePcs<
        ...
        >,
{
    // Generate base STARK challenges (alpha, zeta, zeta_next)
    let base_challenges = BaseStarkChallenges::generate(circuit);
