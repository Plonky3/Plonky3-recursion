...
fn produce_inputs_multi(
    ...
    for &index in &indices {
        ...
        tracing::debug!("bits query {index}: {:?}", bits_one);
    ...
    for (sizes, (commitment, _)) in group_sizes.iter().zip(commitments_and_data.iter()) {
        ...
        tracing::debug!("mats_data = {:?}", mats_data);
...
fn run_fri_test(setup: FriSetup, build_only: bool) {
    ...
    tracing::debug!(
        "n mmcs verifys = {:?}",
        non_primitive_ops_iter.clone().collect::<Vec<_>>().len()
    );
    tracing::debug!("n queries {}", result_1.fri_proof.query_proofs.len());
