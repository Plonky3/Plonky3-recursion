...
use p3_recursion::recursive_traits::{ProofWires, Recursive, RecursiveAir, RecursiveLagrangeSels};
use p3_symmetric::{PaddingFreeSponge, TruncatedPermutation};
use p3_uni_stark::{
    PcsError, Proof, StarkConfig, StarkGenericConfig, SymbolicAirBuilder, SymbolicExpression, Val,
...
fn test_symbolic_to_circuit() {
    ...
    // Get the `alpha` challenge used to fold the constraints, the Lagrange selectors `sels`, as well as the result of folding the constraints.
    ...
    // Build a circuit with wires for `sels`, public values, local values and next values.
    let mut circuit = CircuitBuilder::<F, D>::new();
    let circuit_sels = [
    ...
    let sum = symbolic_to_circuit::<F, Challenge, D>(
    ...
    // Check that the `sum` value generated in the circuit corresponds to the value returned by `verify_with_values`.
