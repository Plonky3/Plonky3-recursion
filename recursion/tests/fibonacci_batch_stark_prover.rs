...
use p3_circuit_prover::batch_stark_prover::{
    OpenInputAirBuilderD4, OpenInputPreprocessor, poseidon2_air_builders_d4,
};
...
fn test_fibonacci_batch_verifier() {
    ...
    // Public values (empty for all circuit tables: Const, Public, Alu, Poseidon2, OpenInput)
    ...
    circuit_builder.enable_open_input::<KoalaBear, D>();
    ...
    let npo_prep: [Box<dyn NpoPreprocessor<F>>; 2] = [
        Box::new(Poseidon2Preprocessor),
        Box::new(OpenInputPreprocessor),
    ];
    let mut npo_air_builders = poseidon2_air_builders_d4();
    npo_air_builders.push(Box::new(OpenInputAirBuilderD4));
