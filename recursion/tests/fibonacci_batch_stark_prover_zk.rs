...
use p3_circuit_prover::{
    BatchStarkProver, CircuitProverData, ConstraintProfile, OpenInputAirBuilderD4,
    OpenInputPreprocessor, Poseidon2AirBuilderD4, Poseidon2Preprocessor, TablePacking,
};
use p3_field::Field;
...
fn test_batch_verifier_zk_hiding_fri() -> Result<(), VerificationError> {
    ...
    circuit_builder.enable_open_input::<F, 4>();
    ...
    let npo_air_builders: Vec<Box<dyn p3_circuit_prover::common::NpoAirBuilder<MyConfig, 4>>> = vec![
        Box::new(Poseidon2AirBuilderD4),
        Box::new(OpenInputAirBuilderD4),
    ];
